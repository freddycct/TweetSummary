...
for e=5:length(events)
    for m=1:length(models)
        ...
        %Use time_series
        if strcmp(models{m}, 'gauss_lda') == 1
            gaussian_topics = load( sprintf('../results/%s/K=%d/%s/gaussian_topics.txt', events{e}, K, models{m} ));
            cmap = jet(K);
            figure;hold on;
            for k=1:K
                p_t = (gaussian_topics(k,1) / sum(gaussian_topics(:,1))) .* (1/sqrt(2 * pi * gaussian_topics(k, 3))) .* exp(-((time_series - gaussian_topics(k, 2)).^2)./(2 * gaussian_topics(k, 3)));
